		...
		foreach($folders as $folder){
			...
			foreach($files as $file){
				...
				if(empty($escaped_name)){
					...
					$new_name = $this->ave->get_file_path(pathinfo($file, PATHINFO_DIRNAME)."/$escaped_name");
	...
	public function ToolAddDirectoryNamePrefixSuffix() : bool {
		...
		foreach($folders as $folder){
			...
			foreach($files as $file){
				$items++;
				if(!file_exists($file)) continue 1;
				$new_name = $this->ave->get_file_path(pathinfo($file, PATHINFO_DIRNAME)."/$prefix".pathinfo($file, PATHINFO_BASENAME).$suffix);
				if(file_exists($new_name) && strtoupper($new_name) != strtoupper($file)){
					$this->ave->write_error("DUPLICATE \"$file\" AS \"$new_name\"");
					$errors++;
				} else {
	...
	public function ToolRemoveKeywordsFromDirectoryName() : bool {
		...
		foreach($folders as $folder){
			...
			foreach($files as $file){
				...
				$new_name = $this->ave->get_file_path(pathinfo($file, PATHINFO_DIRNAME)."/$name");
				if(empty($new_name)){
					$this->ave->write_error("ESCAPED NAME IS EMPTY \"$file\"");
					$errors++;
